	...
	if let Err(e) = async {
		...
		client_alice
			.invite_to_workspace(
				...
				client_bob.current_user().name.clone(),
			)
			.await?;
		client_bob
			.attach_workspace(&client_alice.current_user().name, &workspace_name)
		...
		assert_or_err!(
			client_bob.leave_workspace(&client_alice.current_user().name, &workspace_name)
		);
		assert_or_err!(
			client_alice.leave_workspace(&client_alice.current_user().name, &workspace_name)
		);

		client_alice
			.delete_workspace(workspace_name.clone())
			.await?;
